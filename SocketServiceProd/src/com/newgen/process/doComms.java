...
public class doComms implements Runnable, Constants {
    ...
    private void writeData(final String input) throws IOException {
        try {
            ...
            if (input.length() > 0) {
                ...
                System.out.println("Write out DateTime:  "+ LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
                dataOutputStream.close();
            }
        } catch (Exception i) {
            System.out.println("Exception occurred in write out: "+i.getMessage());
            i.printStackTrace();
        }
    }
    ...
    private String readData(DataInputStream xmlDataInputStream) throws IOException {
        ...
        try {
            ...
            System.out.println("Exception occurred in read data: "+se.getMessage());
            se.printStackTrace();
        }
        return receivedData;
    }
        ...
        try {
            ...
            switch (serviceName){
                ...
                case FREEZE_ACCOUNT:{
                    ...
                    sendFinacleBpmRequest(request, LoadProp.finacleProdBpmUrl,LoadProp.freezeAccountAppCode);
                    break;
                }
                ...
                case REMOVE_LIEN:{
                    ...
                    sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.removeLienAppCode);
                    break;
                }
                case CP_POSTING: {
                    logger.info("welcome to Postng to Finacle on CP");
                    sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.postAppCode);
                    break;
                }
               case FETCH_LIEN: {
                   logger.info("Welcome To FETCH LIEN api call");
                   sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.fetchLienAppCode);
                    break;
                }

                case CURRENT_ACCOUNT:{
                    logger.info("Welcome To FETCH CURRENT ACCT api call");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.fetchOdaAcctAppCode);
                    break;
                }
                case SAVINGS_ACCOUNT:{
                    logger.info("Welcome To FETCH SAVINGS ACCT api call");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.fetchSbaAcctAppCode);
                    break;
                }
                case SPECIAL_ACCOUNT: {
                    logger.info("Welcome To FETCH SPECIAL api call");
                    sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.fetchCaaAcctAppCode);
                    break;
                }
                ...
                case SEARCH_TRANSACTION:{
                    ...
                    if (serviceName.equalsIgnoreCase(SEARCH_TRANSACTION1))
                        sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.searchTransaction2AppCode);

                    else sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.searchTransactionAppCode);

                    break;
                }
                case STAFF_LIMIT: {
                    logger.info("Welcome To Get User Limit");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl, LoadProp.fetchStaffTransactionLimitAppCode);
                    break;
                }
                case BVN_REQUEST: {
                ...
                case DC_STAFF_DETAILS:{
                    ...
                    try {
                       // outputResponseXml = CTEW.callService(request, serviceName);
                        ...
                        String FULL_NAME = xmlParser.getValueOf("tns:FULL_NAME");
                        logger.info("FULL_NAME:::" + FULL_NAME);
                        String DEPARTMENT = xmlParser.getValueOf("tns:DEPARTMENT");
                        logger.info("DEPARTMENT:::" + DEPARTMENT);
                        String COST_CENT = xmlParser.getValueOf("tns:COST_CENT");
                        logger.info("COST_CENT:::" + COST_CENT);
                        String GRADE = xmlParser.getValueOf("tns:GRADE");
                        logger.info("GRADE:::" + GRADE);
                        String phone_num = xmlParser.getValueOf("tns:phone_num");
                        logger.info("phone_num:::" + phone_num);
                        String EMAIL_ADDRESS = xmlParser.getValueOf("tns:EMAIL_ADDRESS");
                        logger.info("EMAIL_ADDRESS:::" + EMAIL_ADDRESS);
                        String REPORTING_MANAGER = xmlParser.getValueOf("tns:REPORTING_MANAGER");
                        logger.info("REPORTING_MANAGER:::" + REPORTING_MANAGER);
                        String REPORTING_MANAGER_NO = xmlParser.getValueOf("tns:REPORTING_MANAGER_NO");
                        logger.info("REPORTING_MANAGER_NO:::" + REPORTING_MANAGER_NO);
                        String jobTitle = xmlParser.getValueOf("tns:JOB");
                ...
                case UPDATE_REMOVE_FINACLE_FLAG:{
                    ...
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.updateRemoveFinacleFlagAppCode);
                    break;
                }
                case VERIFY_CUSTOMER:{
                    logger.info("Welcome TO VERIFY CUSTOMER REQUEST");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.verifyCustomerAppCode);
                    break;
                }
                case UPDATE_RETAIL_RISK_SCORE:{
                    logger.info("Welcome TO UPDATE Retail customer risk score");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.updateRetailRiskScoreAppCode);
                    break;
                }
                case UPDATE_CORP_RISK_SCORE:{
                    logger.info("Welcome TO UPDATE Retail customer risk score");
                    sendFinacleFiRequest(request,LoadProp.finacleProdFiUrl,LoadProp.updateCorpRiskScoreAppCode);
                    break;
                }
                case PLACE_LIEN:{
                    logger.info("welcome to Place Lien API");
                    sendFinacleBpmRequest(request,LoadProp.finacleProdBpmUrl,LoadProp.placeLienAppCode);
                    break;
                }
